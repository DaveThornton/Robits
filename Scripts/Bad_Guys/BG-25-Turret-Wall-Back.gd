...
export var camera_activation = false
export var camera_activation_time = 2.1
...
onready var camera_activation_timer = $Timer_Camera_Activation
...
func _process(delta):
	...
	if current_look_time > look_time && activated:
		current_look_time = 0
...
func set_camera_activation(_act): 
	camera_activation = _act
	print_debug("setting camera activation in turret ", _act)

		
func triggered():
	if camera_activation:
		print_debug("triggered")
		camera_activation_timer.start()
...
func _on_Timer_Camera_Activation_timeout(): activate(0)
