...
func _ready():
	...
	if test1 != 0:
		print("error arcade player select connect input to load map")
	if test2 != 0:
		print("error arcade player select connect input to screen")
...
#		get_tree().get_current_scene().load_screen(map_screen)# this spot !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		queue_free()
	if num_with_s == 0:
